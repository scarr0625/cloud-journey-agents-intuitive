...
A Google ADK orchestrator with an existing interactive Journey lifecycle and
separate non-interactive batch jobs implementing
[the durable-state design](../../durable_state_confluence.md). Journey business records,
batch checkpoints, and conversation sessions use separate databases. The
interactive flow remains available; the new batch flow ends at readiness.

## Batch durable state

| Database | Contents | Runtime owner |
| --- | --- | --- |
| `cloud-journey-db` | Journeys, business events, `journey_operation_status`, `journey_external_dependency`, legacy command outcomes | Business operations / Data API adapter |
| `durable-state-db` | `agent_execution`, `operation_checkpoint`, `checkpoint_event` | APM Validation, AD Provisioning, App Factory Helper jobs only |
| `session-db` | ADK-managed conversation metadata, events, state | Orchestrator and Chat Assistant session persistence |

Checkpoint statuses are exactly `PENDING`, `RUNNING`, `WAITING`, `COMPLETED`, and
`FAILED`. They are independent of Journey business states and job invocation
results. Checkpoints retain their operation key, stage, external request ID,
result reference, and error. Each save appends a history event atomically.
Submission and polling update the same AD operation checkpoint. A job can end
with `SUCCEEDED` while its operation remains `WAITING`.

The job reads the business outcome before acting. If a previous invocation
committed that outcome but died before saving its checkpoint, a retry reconciles
the saved business result. The simulated MyAccess operation uses a stable
idempotency key and polls the original request. Database uniqueness, version
checks, and a five-minute execution lease prevent overlapping owners from
overwriting one another. A crashed invocation can be recovered after its lease
expires; invocations must finish within that lease. A real external adapter must
support provider idempotency or request reconciliation before retrying submission.

The declared sequence is APM validation, AD submission, AD polling (repeated
while pending), then App Factory validation. The last operation completes with
`READY_TO_PROVISION` or `APP_FACTORY_VALIDATION_ERROR`; validation errors are
recorded business results, while runtime failures produce `FAILED` checkpoints.
The simulated App Factory validator requires a nonempty captured `inventory`.
Cloud Build and subsequent provisioning are outside this batch flow.

`POST /v1/chat/query` exposes a separate Chat Assistant with only authorized
status tools. `POST /v1/query` retains the interactive orchestrator. Both persist
conversation history in Session DB, scoped by application, verified user, and
session. Updating verified identity claims preserves the conversation history.
Status tools read business records and never open the checkpoint store.

**PoC integration boundary:** `LocalBusinessGateway` is an in-process stand-in
for private MCP and the Data API. `workflows.local_workflow` is a local Workflows simulator
that launches each job in a separate process and waits for it. This repository
does not deploy private MCP, a network Data API, Google Workflows, or real
MyAccess/App Factory integrations. The `BusinessGateway` interface is the seam
for replacing the local adapter. The read-only Chat Assistant has no mutation
tools; the existing interactive orchestrator deliberately retains its tools.

### Configure and run

For a new local PostgreSQL instance, `docker compose up -d` creates all three
databases. Set the three URLs from `.env.example`. An existing Journey database
may keep its current name and `DATABASE_URL`; no business rows are moved or
converted into checkpoints.

Docker initialization scripts run only for a fresh volume. To add missing
databases to an existing local volume without deleting its contents:
...
`000` through `004`. ADK creates and manages the session tables separately.
The PoC also creates missing tables at job startup; production roles can instead
use pre-applied migrations.

Start with an existing Journey at `CREATED`/`APM_VALIDATED` with its request
context already captured, or an interactive Journey at `APPROVED`. The normal
interactive inventory/plan flow still requires its external approval before AD
submission. Do not run the legacy execution simulator and the batch workflow
concurrently on the same Journey.

```powershell
python -m workflows.local_workflow --journey-id J-YOUR-ID --workflow-run-id example-run
```

For individual job invocations (all four can share one workflow run ID):

```powershell
python -m journey_poc.batch_job --agent apm-validation-agent --journey-id J-YOUR-ID --workflow-run-id example-run
python -m journey_poc.batch_job --agent ad-provisioning-agent --mode submit --journey-id J-YOUR-ID --workflow-run-id example-run
python -m journey_poc.batch_job --agent ad-provisioning-agent --mode poll --journey-id J-YOUR-ID --workflow-run-id example-run
```

Repeat the polling invocation until its JSON result reports `COMPLETED`, then
run `app-factory-helper-agent`. By default the first simulated poll is pending
and the second completes; `SIMULATED_MYACCESS_PENDING_POLLS` controls this.
The local workflow bounds polling with `--max-polls` and returns `WAITING` when
the limit is reached. Running it again resumes the same operation.

The following sections describe the preserved interactive lifecycle and demos.
...
6. **An approved Journey resumes.** Cloud Compass observes the durable approval and
   ...
   deployment validation. Each business transition is committed before the next one begins.
...
records legacy command outcomes. Batch checkpoints live in `durable-state-db`.
ADK stores conversation history, context, and verified identity claims in
`session-db`; it never stores the raw ID token. After a restart, business status
is rebuilt from Journey DB independently of the conversation session.
...
- Central transition validation in `examples/local-business/journey_poc/cloud_journey/state_machine.py`
...
- Separate batch execution/checkpoint/history tables with restart recovery
- Business-facing operation and external-dependency status records
- Persistent ADK sessions and a separate read-only Chat Assistant endpoint
...
- One `journey_poc.main.root_agent` composing two specialist-routing tools
...
```text
DATABASE_URL=postgresql+psycopg://journey:URL_ENCODED_PASSWORD@127.0.0.1:5432/cloud-journey-db
DURABLE_DATABASE_URL=postgresql+psycopg://journey:URL_ENCODED_PASSWORD@127.0.0.1:5432/durable-state-db
SESSION_DATABASE_URL=postgresql+psycopg://journey:URL_ENCODED_PASSWORD@127.0.0.1:5432/session-db
```

### Apply business migrations

Apply the SQL files in `migrations/business` to the configured business
database with `psql -v ON_ERROR_STOP=1`. The additional durable migration belongs
only in `durable-state-db`, and ADK manages `session-db`. Existing business data
stays in its current database; keep its existing URL if it is named
`durable_journey`.
...
6. `005_business_operation_progress.sql` adds business-facing operation results
   and external dependency progress without altering existing Journey records.

After configuring all three databases, start the agent:

```powershell
uvicorn journey_poc.main:app --reload
```
...
`APP_FACTORY_READY` are stable business milestones in the interactive flow. The other nonterminal states
...
| `JourneyDataChanged` | User/System Data updated | Inventory, plan, or execution-outcome context is persisted |
...
simulator is a separate module, `journey_poc.cloud_journey.approval_backend`. All state changes
...
```powershell
python -m journey_poc.cloud_journey.approval_backend J-XXXXXXXX --decision approve --reviewer reviewer --delay-seconds 60
```
...
```powershell
python -m journey_poc.cloud_journey.approval_backend J-XXXXXXXX --decision reject --reviewer reviewer --delay-seconds 60 --reason "Network firewall design is incomplete"
```
