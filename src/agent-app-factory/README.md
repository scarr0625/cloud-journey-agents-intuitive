...
```sh
docker build -f src/agent-app-factory/Dockerfile -t agent-app-factory .
```
...
The Python package is `agent_app_factory` (the package manifest maps `app/` to this
...
```sh
uvicorn agent_app_factory.server:app --host 0.0.0.0 --port 8080
```
